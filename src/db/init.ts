...
export const initDB = async () => {
    ...
    const currProducts = await query('SELECT * FROM PRODUCTS');
    if (currProducts.rowCount === 0){
        products.forEach(
            async (product): Promise<void> => {
                const string = `INSERT INTO PRODUCTS(product_id, name, price, url, description) VALUES(${product.id}, '${product.name}', ${product.price}, '${product.url}', '${product.description}');`; 
                await query(string);
            }
        );
    }

    const currUsers = await query('SELECT * FROM USERS');
    if(currUsers.rowCount === 0) {
        // You can customize password in .env
        await query(`INSERT INTO USERS(firstName, lastName, password) VALUES('admin', 'user', '${process.env.FRONTENDPASSWORD}');`)
    }
};
